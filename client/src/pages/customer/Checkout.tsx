...
const checkoutSchema = z.object({
  ...
  // Payment Information (dummy for demo)
  cardNumber: z.string().min(16, "Card number must be 16 digits"),
  expiryDate: z.string().min(5, "Expiry date required (MM/YY)"),
...
export default function Checkout() {
  ...
  const createOrderMutation = useMutation({
    mutationFn: async (data: CheckoutFormData) => {
      const orderData = {
        vendorId: cartItems[0]?.product.vendorId, // Assuming single vendor for simplicity
        status: "pending",
    ...
    onSuccess: (order) => {
      toast({
        ...
        description: `Your order #${order.id} has been placed and will be prepared shortly.`,
      });
  ...
  if (cartItems.length === 0) {
    return (
      <div className="text-center py-16 space-y-6">
        <h2 className="text-2xl font-bold text-foreground">No items to checkout</h2>
